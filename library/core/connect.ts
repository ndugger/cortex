...
export default function connect<Constructor extends Node>(element: VirtualElement<Constructor>, host: Node): void {
    ...
    if (!element.node) {
        console.log('f', element.constructor)
    ...
    if (element.properties) for (const option of Object.keys(element.properties)) {
        ...
        if (option === 'namespaces') for (const [ name, space ] of Object.entries(element.properties[ option ])) {
            (element.node as Element).setAttributeNS('http://www.w3.org/2000/xmlns/', `xmlns:${ name }`, space);
            continue;
        }

        if (option === 'attributes') {

            for (const attribute of Array.from((element.node as Element).attributes)) {

                if (!(attribute.name in element.properties.attributes) || element.properties.attributes[ attribute.name ] === false) {
                    (element.node as Element).removeAttributeNode(attribute);
                }
            }
            ...
            for (const [ attribute, object ] of Object.entries(element.properties.attributes)) {
                ...
                if (object === true) {
                    (element.node as Element).setAttribute(attribute, '');
                    continue;
                }
                ...
                if (typeof object === 'object') for (const [ key, value ] of Object.entries(object)) {
                    ...
                    if ('namespaces' in element.properties && attribute in element.properties.namespaces) {
                        (element.node as Element).setAttributeNS(element.properties.namespaces[ attribute ], `${ attribute }:${ key }`, value.toString());
                    }
                    else {
                        const root = (host as Element).querySelector(`xmlns:${ attribute }`);

                        if (root) {
                            (element.node as Element).setAttributeNS(root.getAttribute(`xmlns:${ attribute }`), `${ attribute }:${ key }`, value.toString());
                        }
                        else {
                            (element.node as Element).setAttributeNS(null, key, value.toString());
                        }
                    }
                }
                else {
                    (element.node as Element).setAttribute(attribute, object as string);
                }
