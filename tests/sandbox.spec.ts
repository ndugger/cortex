...
class TestButton extends Quark.Widget {
    ...
    public render(): Quark.Node[] {
        ...
        return [
            new Quark.Node(HTMLButtonElement, { className: 'xyz', onclick: () => this.handleButtonClick() }, [
                new Quark.Node(HTMLSlotElement),
                new Quark.Node(HTMLSpanElement, { attributes: { urmom: { foo: 'bar' } }, namespaces: { 'urmom': 'gay' }, style: { fontWeight: 'bold' }, textContent: `(clicked: ${ clicked })` })
            ])
        ];
    }
}
...
class TestContainer extends Quark.Widget<{ foo: boolean }> {
    ...
    public render(): Quark.Node[] {
        ...
        return [
            new Quark.Node(TestButton, { attributes: { foo: 'bar' } }, [
