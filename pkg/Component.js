...
class Component extends CustomHTMLElement {
  /**
   * Creates a component, attaches lifecycle listeners upon instantiation, and
   * initializes shadow root
   */
  constructor() {
    super();
    this.addEventListener('componentconnect',
                          event => this.handleComponentConnect(event));
    this.addEventListener('componentcreate',
                          event => this.handleComponentCreate(event));
    this.addEventListener('componentdisconnect',
                          event => this.handleComponentDisconnect(event));
    this.addEventListener('componentready',
                          event => this.handleComponentReady(event));
    this.addEventListener('componentrender',
                          event => this.handleComponentRender(event));
    this.addEventListener('componentupdate',
                          event => this.handleComponentUpdate(event));
  ...
  updatedCallback() {
    branch.push(this);
    const style = createElement_1.createElement(HTMLStyleElement,
                                                {textContent : this.theme()});
    const elements =
        this.render().concat(style).map(mapChildToElement_1.mapChildToElement);
    /**
    ...
    if (!this[layout]) {
      ...
      this[layout] =
          mergeTreeChanges_1.mergeTreeChanges(this[layout], elements);
    }
    /**
  ...
  getContext(dependency) {
     ...
     */
    if (!found) {
      // TODO (delay functional render so context is rendered before functions
      // called) throw new Error(`Missing context: ${ dependency.name }`)
    }
 ...
 * @param node
 */
function isComponent(node) { return node instanceof Component; }
 ...
 * @param constructor
 */
function isFn(constructor) { return !isConstructor(constructor); }
Component.isFn = isFn;
/**
 * Decides if a node is a portal mirror
 * @param node
 */
function isMirror(node) { return node instanceof Component.Portal.Mirror; }
Component.isMirror = isMirror;
/**
 * Used to provide contextual state within a given component tree
 */
class Context extends Component {
  render() { return [ createElement_1.createElement(HTMLSlotElement) ]; }
  theme() {
    return `
                :host {
                    display: contents;
...
class Portal extends Component {
  ...
  constructor() {
    ...
    if (!portals.has(this.constructor)) {
      ...
      (_a = portals.get(this.constructor)) === null || _a === void 0
          ? void 0
          : _a.push(this);
    }
  }
...
class Mirror extends Fragment_1.Fragment {
  reflect() {
    var _a;
    for (const portal of (_a = portals.get(this.target)) !== null &&
                 _a !== void 0
             ? _a
             : []) {
      portal.append(this);
