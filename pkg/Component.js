...
class Component extends CustomHTMLElement {
    ...
    updatedCallback() {
        ...
        if (!this[layout]) {
            ...
            this[layout] = mergeTreeChanges_1.mergeTreeChanges(
                this[layout],
                elements
            );
        }
        ...
        for (const element of this[layout])
            if (element) {
                if (!element.node) {
                    element.node = createDocumentNode_1.createDocumentNode(
                        element
                    );
                }
                connectElementToHost_1.connectElementToHost(
                    element,
                    this.shadowRoot
                );
            }
...
(function (Component) {
    ...
    class Context extends Component {
        render() {
            return [createElement_1.createElement(HTMLSlotElement)];
        }
        theme() {
            return `
                :host {
