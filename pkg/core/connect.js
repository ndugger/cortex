...
export function connect(element, host) {
  ...
  } else if ((Element.isNative(element) || Element.isCustom(element)) &&
             element.properties) {
    for (const property of Object.keys(element.properties)) {
      ...
      if (property === 'namespaces') {
                for (const [name, space] of Object.entries(element.properties[property] ?? {})) {
                  element.node?.setAttributeNS(XML_NAMESPACE, `xmlns:${name}`,
                                               space);
                }
      }
      /**
